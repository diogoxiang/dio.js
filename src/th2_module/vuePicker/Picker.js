/**
 * https://gitee.com/tomxiang/mobile-picker
 * 说明 文件
 */
;
(function (global) {
  ...
  Picker.prototype = {
    ...
    addList: function (parent, data) {
      var html = '',
        that = this;
      var index = 0,
        scrollIndex = parent.parentNode.scrollIndex,
        text = '';
    ...
    setLock: function (value) {
      var confirm = Util.getEle(this.container, '.mp-confirm'),
        old = this.lock;
    ...
    bindEvent: function () {
      ...
      ['touchstart', 'touchend', 'touchmove'].forEach(function (action) {
        that.box.parentNode.addEventListener(action, function (event) {
          ...
          var scrollHeight
          if (child.length >= 2) {
            scrollHeight = child[child.length - 2].offsetTop;
          }
